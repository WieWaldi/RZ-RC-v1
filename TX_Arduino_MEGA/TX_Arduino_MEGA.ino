/*+----------------------------------------------------------------------------+
  | RZ-RC-v1 -- TX_Arduino_Nano                                                |
  +----------------------------------------------------------------------------+
  |       Usage: ---                                                           |
  | Description: https://github.com/WieWaldi/RZ-RC-v1                          |
  |                                                                            |
  |              This sketch is for a Arduino Nano paired with analogue        |
  |              joysticks and a 128x64 oLED Display.                          |
  |                                                                            |
  |    Requires: ---                                                           |
  |       Notes: ---                                                           |
...
// --------------------- Settings ----------------------------------------------
        ...
        // Misc settings
        const boolean debug                     = false;                         // Debugging on/off
        ...
        const int displayMaxLines               = 5;                            // Maximum number of display lines for menu items
...
struct MenuMenus {
  uint32_t lastMenuActivity = 0;                                                  // Time of last menu activity
  String menuTitle = "";                                                              // Title of the current menu
  String menuItems[10];                                                          // Array to hold menu item strings
  int noOfItems = 0;                                                            // Number of items in the current menu
  int selectedMenuItem = 0;                                                         // Currently selected menu item
  int highlightedItem = 0;                                                      // Currently highlighted menu item
  int mValueEntered = 0;                                                            // Value entered in value entry mode
  int mValueLow = 0;                                                                // Minimum value for value entry
  int mValueHigh = 0;                                                               // Maximum value for value entry
  int mValueStep = 0;                                                               // Maximum value for value entry
};
MenuMenus MenuMenu;                                                                 // Create Menu object
...
void drawMenu() {
  ...
  if (MenuMenu.highlightedItem > MenuMenu.noOfItems) {
    MenuMenu.highlightedItem = MenuMenu.noOfItems;
  }
  if (MenuMenu.highlightedItem < 1) {
    MenuMenu.highlightedItem = 1;
  }
  display.setCursor(5, 2);                                                   // Cursorposition x, y
  display.println(MenuMenu.menuTitle);
  display.drawFastHLine(0,10,128,SSD1306_WHITE);                           // Draw horizontal line
  display.setCursor(0, topLine);                                                     // Cursorposition x, y
  for (int i=1; i <= displayMaxLines; i++) {
    int item = MenuMenu.highlightedItem - (displayMaxLines / 2 + 1) + i;
    if (item == MenuMenu.highlightedItem) {
      display.setTextColor(BLACK, WHITE);
    } else {
      display.setTextColor(WHITE);
    }
    if (item > 0 && item <= MenuMenu.noOfItems) {
      display.println(MenuMenu.menuItems[item]);
    } else {
      display.println(" ");
    }
  }
...
void updateScreen(){
  ...
  if ( (unsigned long)(millis() - MenuMenu.lastMenuActivity) > (menuTimeout * 1000) ) {  // Check for menu timeout and reset if necessary
    menuMode = off;
    return;
  }
  switch (menuMode) {
...
void menuReset() {                                                              // Reset menu variables and flags 
  rotaryPosition = 0;
  rotaryButtonPressed = false;
  rotaryButtonSignal = false;
  menuMode = menu;
  MenuMenu.selectedMenuItem = 0;
  MenuMenu.noOfItems = 0;
  MenuMenu.menuTitle = "";
  MenuMenu.highlightedItem = 0;
  MenuMenu.mValueEntered = 0;
  MenuMenu.lastMenuActivity = millis();                                         // log time
}
...
void bootScreen(){
  ...
  for (int i = 1; i < 97;)  {
    ...
    i = i + 4;
                                                                                ...
                                                                                //    ...
void mainMenu() {
  menuReset();                          // clear any previous menu
  menuMode = menu;                      // enable menu mode
  MenuMenu.noOfItems = 7;           // set the number of items in this menu
  MenuMenu.menuTitle = "menu";     // menus title (used to identify it)
  MenuMenu.menuItems[1] = "S";      // set the menu items
  MenuMenu.menuItems[2] = "C";
  MenuMenu.menuItems[3] = "C";
  MenuMenu.menuItems[4] = "C";
  MenuMenu.menuItems[5] = "C";
  MenuMenu.menuItems[6] = "C";
  MenuMenu.menuItems[7] = "S";

  // MenuMenu.menuItems[1] = "Screen Off";      // set the menu items
  // MenuMenu.menuItems[2] = "Chan Info 1+2";
  // MenuMenu.menuItems[3] = "Chan Info 3+4";
  // MenuMenu.menuItems[4] = "Chan Info All";
  // MenuMenu.menuItems[5] = "Chan Info Aux";
  // MenuMenu.menuItems[6] = "Chan Config";
  // MenuMenu.menuItems[7] = "Settings";
